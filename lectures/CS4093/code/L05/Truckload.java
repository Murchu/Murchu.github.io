...
/**/
		...
		for (int i = 0; i<n; i++) {
			load[i] = VariableFactory.enumerated("load"+i, 0,  stock[i], solver); //how many of each type
		}
		...
		//solver.post(IntConstraintFactory.scalar(load, weights, "<=", VF.fixed(capacity, solver)));
		...
		//solver.post(IntConstraintFactory.scalar(load, profits, ">", VF.fixed(cost, solver)));
		...
		System.out.println();
		System.out.println("profit upper bound= " + maxprofit);
